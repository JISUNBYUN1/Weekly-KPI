...
with tab2:
    ...
    # W34로 고정 (샘플 데이터 기준)
    selected_week = "W34"
    st.info(f"📊 현재 {selected_week} 데이터를 표시 중입니다")
    
    # KPI 요약
    st.subheader(f"📊 {selected_week} KPI 요약")
    
    forecast_values = [sample_data[f"{p}_{selected_week}"]['forecast'] for p in partners]
    sales_values = [sample_data[f"{p}_{selected_week}"]['sales'] for p in partners]
    premium_values = [sample_data[f"{p}_{selected_week}"]['premium'] for p in partners]
    efficiency_values = [sample_data[f"{p}_{selected_week}"]['efficiency'] for p in partners]
